...
>*Answer 1:* The vegetation recovers over time, increasing in 'green-ness' as measured by NDVI rapidly after a fire but reaches a maximum level of green-ness ~ a saturation level where the vegetation has completely recovered. 
...
>*Answer 2:* Estimates for alpha (NDVI time at 0) and gamma (max average increase in NDVI), are very similar between the simpler and full model.
Lambda (the rate of increase of NDVI) shows a bit more of a difference between the simple model (average increase in NDVI) vs the full model, which takes seasonality into account; where lamda of the simpler model equal to 1.17687 and the full model equal to 0.989154. Becauase the full model incorporates seasonality, the decrease in NDVI during the dry seasons result in a smaller lambda value (rate of increase) for the full model relative to the simpler model. The standard errors show the same trend, with similar values for alpha and gamma and lower values in the full model for lambda. 
...
>*Answer 3:* The model that minimizes the residual sum of squares is better, in this case Model 2 - the full model. 

Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

>*Answer 4:* Degrees of freedom are the number of independent values in an analysis. In this case there are 2 degrees of freedom in the ANOVA because 2 is the number of differnces in independent values between the models. 
...
>*Answer 5:* Yes, model 2 - the full model is the preferred model as it has the lowest AIC value. 
...
>*Answer 6:* AIC values are calculated under the assumption that the log likelihood has been maximized. In Maximum Likelihood Estimation this assumption is met, but under Non-Linear Least Squares it is not, and so while AIC values can still be computed for NLS models they should not be compared or interpreted. 
